...
{
	/// <summary>
	/// Service: Converts Markdown to HTML, supporting edge-cases and modern GFM extensions.
	/// Allows injection of a class attribute for all tags.
	/// </summary>
	public class MarkdownToHtmlConverterService : IMarkdownToHtmlConverterService
	{
		public string Convert(string markdown, string className = "", bool escapeMarkdown = false)
		{
			...
			// Paragraphs for "orphan" lines not inside block tags
			markdown = Regex.Replace(markdown,
				 @"^(?!<.*>)([^\n]+)$",
				 delegate (Match m)
				 {
		...
		public async Task ConvertStreamAsync(TextReader input, TextWriter output, string className = "", bool escapeMarkdown = false, CancellationToken cancellationToken = default)
		{
			// Čteme vše do stringu (pro skutečně obří data lze optimalizovat na bloky)
			var sb = new StringBuilder();
